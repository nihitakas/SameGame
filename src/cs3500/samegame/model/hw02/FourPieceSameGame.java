...
/**
 * Represents a game of Four-Piece Same Game.
 */
public class FourPieceSameGame implements SameGameModel<MyPiece> {

  private boolean gameStarted;
  protected List<List<MyPiece>> board;
  int swaps;
  int score;


  /**
   * Constructor a FourPieceSameGame with default settings.
   * default is game not started yet
   */
  ...
  public void swap(int fromRow, int fromCol, int toRow, int toCol) {
    ...
    if (this.swaps == 0) {
      throw new IllegalStateException("No swaps left.");
    }

    if (fromRow < 0 || fromRow >= board.size() || fromCol < 0 || fromCol >= board.get(fromRow)
            .size()
            || toRow < 0 || toRow >= board.size() || toCol < 0 || toCol
            >= board.get(toRow).size()) {
  ...
  private int dfs(int row, int col, MyPiece pieceColor, boolean[][] visited) {
  ...
  private int calculateMatchScore(int count) {
    if (count < 3) {
      return 0;
    } else {
      return (count) - 2;
  ...
  private void intializeBoard(int rows, int cols, boolean random) {
  ...
  private boolean isValidBoard(List<List<MyPiece>> board) {
